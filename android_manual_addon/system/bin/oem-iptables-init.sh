...
# FORBID privoxy from ANY but lo, and ssh from Cellular facing
$IPTABLES $IPTABLES_OPTS -A XX_INPUT  ! -i lo     -p tcp --syn -m tcp --dport 8118 -j DROP
$IPTABLES $IPTABLES_OPTS -A XX_INPUT  ! -i lo     -p tcp --syn -m tcp --dport 8228 -j DROP
...
$IPTABLES $IPTABLES_OPTS -A XX_INPUT  -p udp -m udp --dport 9993 -j ACCEPT
$IPTABLES $IPTABLES_OPTS -A XX_OUTPUT -p udp -m udp --sport 9993 -j ACCEPT
$IPTABLES $IPTABLES_OPTS -A XX_OUTPUT -p udp -m udp --dport 9993 -j ACCEPT
# PERMIT new DHCP
